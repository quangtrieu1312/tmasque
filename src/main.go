...
// fullTunnel is set true when the server advertises a 0.0.0.0/0 route (full-tunnel).
var fullTunnel bool

// delSuppressRule removes the `table main suppress_prefixlength 0` rule used for
// full-tunnel routing, so neither a clean nor an unclean exit leaves the host's
// default route suppressed (which would break normal routing after we're gone).
// Best-effort; loops in case duplicates accumulated.
func delSuppressRule() {
    for i := 0; i < 8; i++ {
        if err := exec.Command("/sbin/ip", "rule", "del", "table", "main", "suppress_prefixlength", "0").Run(); err != nil {
            break
        }
    }
}
...
func Bootstrap(ctx context.Context) {
    ...
    // Full-tunnel (a 0.0.0.0/0 route): the rule above sends every non-marked packet to
    // table 9000 — for a default route that's the local subnet and the SSH/management
    // path too, which breaks them (asymmetric return: inbound direct, outbound via the
    // VPN). suppress_prefixlength 0 keeps main's specific/connected routes authoritative
    // and lets only the default fall through to table 9000. Added AFTER the rule above so
    // it gets a lower pref and is evaluated first (the wg-quick approach).
    if fullTunnel {
        if logger.ShouldLog(logger.INFO) { logger.Info("Full-tunnel: /sbin/ip rule add table main suppress_prefixlength 0") }
        if err := exec.Command("/sbin/ip", "rule", "add", "table", "main", "suppress_prefixlength", "0").Run(); err != nil {
            logger.Fatal(fmt.Sprintf("Error adding suppress_prefixlength rule: %v", err))
        }
    }
}
...
func establishTunTapAndRoutes(ctx context.Context, routes []connectip.IPRoute, localPrefixes []netip.Prefix) ([]*water.Interface, error) {
	...
	for _, route := range routes {
		...
		for _, prefix := range route.Prefixes() {
            if prefix.String() == "0.0.0.0/0" { fullTunnel = true }
